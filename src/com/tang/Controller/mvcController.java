...
//		os.write(arg0);
	...
	public void getVersion(HttpServletRequest request, HttpServletResponse response){
		...
		try {
			JSONObject json = new JSONObject();
			//Dex下载地址  只存在一种情况 
			json.put("patch_url", "/MySpringWeb/mvc/getDex");
			//app 版本号
			json.put("version_code", "1.0");
			//app 补丁号
			json.put("version_patch","1.0");
			//app 升级文本内容
			json.put("remark","这次我们修复了一些既有的bug,同时增添了一些新的功能.....");
			// 1 表示提示升级 2 表示强制升级 3不升级（最新版本）
			json.put("version_type", "3");
			//app 下载地址
			json.put("app_url", "/MySpringWeb/mvc/getApp");
			//目前只有两种情况  *.apatch (andFix)   *.apk（Tinker）
 			json.put("patch_name", "out.apatch");
			ServletOutputStream os = response.getOutputStream();
			os.write(json.toString().getBytes("UTF-8"));
